...
mongoose.connect('mongodb+srv://richa:thakur@cluster0.rr6eg.mongodb.net/myFirstDatabase?retryWrites=true&w=majority', {useNewUrlParser: true, useCreateIndex: true})
...
app.get('/apply', (req, res) => {
  return res.render('application', {message: 'nothing'})
})
...
app.post('/postcandidate', (req, res) => {
  ...
  // set interviewStatus to applied here
  body.rollNumber=body.rollNumber.toUpperCase();
  var candidates = new Candidates(body);
  candidates.save().then((cand) => {
    res.render('application', {message: 'saved'})
  }).catch((e) => {
    res.render('application', {message: e})
  });
